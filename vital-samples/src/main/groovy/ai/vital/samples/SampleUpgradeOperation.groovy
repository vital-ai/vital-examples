...
/**
 * This class demonstrates how to upgrade live data segment in vitalprime
...
class SampleUpgradeOperation {
	...
	static def main(args) {
		...
		File oldDataFile = new File(resourcesDir, "ml-10K_all.v_1_0_0.vital")
//		File newDataFile = new File(resourcesDir, "ml-10K_all.v_1_0_1.vital")
		
		File outNewDataFile = new File(resourcesDir, "ml-10K_all.v_1_0_1.segment-upgraded.vital")
//		File outOldDataFile = new File(resourcesDir, "ml-10K_all.v_1_0_0.segment-downgraded.vital")
		
		File builderFile = new File(resourcesDir, "movielens-upgrade.groovy")
...
//		serviceAdmin.addSegment(app, lvs2, true)
		...
		//load older domain version
		loadDomainJar(serviceAdmin, app, resourcesDir, oldJarName)

		FileInputStream fis = new FileInputStream(oldDataFile)
		...
		//load newer domain in order to execute upgrade operation
		...
		UpgradeOptions upgradeOpts = ops.getUpgradeOptions()
		...
		println "Upgrade status " + status.status + " - " + status.message
		
		
		FileOutputStream fos = new FileOutputStream(outNewDataFile)
