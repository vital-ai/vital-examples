...
var ADMIN_APP_ID = 'apps-admin'

var APP_ID = 'vertx-app-sample';
	
if(window.location.hostname.indexOf('dev.') == 0) {
	ADMIN_APP_ID = 'commerce2-admin'
	APP_ID = 'app';
	console.warn("DEV endpoint");
}

var ENDPOINT = 'endpoint.' + ADMIN_APP_ID;


//from vitalserviceadmin
var ADMIN_APP_OBJECT = null;
var APP_OBJECT = null;
...
var URL_PREFIX = '/vertx-admin-app-sample';
	...
	router.on({
	  ...
	  '*': function (params) {
		  ...
		  if(LOGGED_IN) {
			  ...
			  ctx.username = vitalserviceadmin.getCurrentLogin() ? vitalserviceadmin.getCurrentLogin().get('username') : '';
...
$(function(){
	...
	vitalserviceadmin = new VitalServiceAdmin(ENDPOINT, EVENTBUS_URL, function(){
		
		console.log('connected to endpoint');
	
		vitalserviceadmin.getApp(ADMIN_APP_ID, function(appObj){
			
			if(appObj == null) {
				console.error("Admin app not found: " + ADMIN_APP_ID);
				return;
			}
			
			ADMIN_APP_OBJECT = appObj;
			
			vitalserviceadmin.getApp(APP_ID, function(appObj){
				...
				console.error("Error when getting app " + APP_ID + " : " + error);	
			});
			
		}, function(error){
			
			console.error("Error when getting admin app " + ADMIN_APP_ID + " : " + error);
...
var doLogout = function() {
	...
	vitalserviceadmin.callFunction(ADMIN_APP_OBJECT, VitalServiceWebsocketImpl.vitalauth_logout, {}, function(logoutSuccess){
...
var doLogin = function() {	
	...
	vitalserviceadmin.callFunction(ADMIN_APP_OBJECT, VitalServiceWebsocketImpl.vitalauth_login, {loginType: 'AdminLogin', username: username, password: password}, function(loginSuccess){
...
var refreshLoginState = function() {
	...
	var currentLogin = vitalserviceadmin.getCurrentLogin();
...
function handleSearch(mainPanel, params, searchType) {
	...
	if(searchType == 'search') {
		...
		vitalserviceadmin.query(APP_OBJECT, queryBuilder, function(seachResults){
		...
		//get object
		vitalserviceadmin.get(APP_OBJECT, {_type: 'ai.vital.vitalsigns.model.property.URIProperty', value: uri}, false, function(getResults){
			...
			vitalserviceadmin.graphQuery(APP_OBJECT, queryBuilder, function(graphResults){
...
//		</table>
	...
	vitalserviceadmin.get(APP_OBJECT, {_type: 'ai.vital.vitalsigns.model.property.URIProperty', value: uri}, false, successHandler, function(errorMsg){
