...
class Clarifai_ProcessImageHandler extends VertxHandler {

	public Clarifai_ProcessImageHandler() {
		super(null);
	}
		...
		try {
			...
			if(part == total) {
				...
				ResultList qrl = vitalService.callFunction('commons/scripts/Aspen_Usage',
					[action: 'incrementUsage', key:'clarifai', increment: 1, limit: 1000])
				...
				rl = vitalService.callFunction('commons/scripts/Aspen_Predict', 
					['inputBlock': [img], 'modelName': 'clarifai-image-categorization'])
