package ai.vital.clarifai.app.main
...
class ClarifaiAppMain {

	
	def static main(args) {
		
		println "Clarifai App - Vertx 3"
		
		if(args.length != 1) {
			System.err.println('usage: clarifai-app <conf_file>')
			System.exit(-1)
			return
		}
		...
		new ClarifaiAppMain().init(config).startVitalService()
	...
	public ClarifaiAppMain() {
		vertx = Vertx.vertx()
	}
	
	ClarifaiAppMain init(Config config) {
	...
	def startClarifaiAppVerticle() {
		
		vertx.deployVerticle("groovy:" + ClarifaiAppVerticle.class.canonicalName, 
			[
			...
			if(!res.succeeded()) {
				...
				System.err.println("${ClarifaiAppVerticle.class.canonicalName} init failed: " + res.cause())
				System.exit(-1)
			}
			
			println "${ClarifaiAppVerticle.class.canonicalName} started: ${res.result()}"
...
//		outboundPermitted : List<PermittedOptions>
		...
		def inboundPermitted1 = [
			address: ai.vital.auth.vertx3.VitalJSEndpointsManager.ENDPOINT_PREFIX + ClarifaiAppVerticle.appID
		]
