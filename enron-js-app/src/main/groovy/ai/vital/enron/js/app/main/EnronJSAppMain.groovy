package ai.vital.enron.js.app.main
...
class EnronJSAppMain {

	
	def static main(args) {
		
		println "Enron JS App - Vertx 3"
		
		if(args.length != 1) {
			System.err.println('usage: enronjs-app <conf_file>')
			System.exit(-1)
			return
		}
		...
		new EnronJSAppMain().init(config).startVitalService()
	...
	public EnronJSAppMain() {
		vertx = Vertx.vertx()
	}
	
	EnronJSAppMain init(Config config) {
	...
	def startEnronJSAppVerticle() {
		
		println "Starting ${EnronAppVerticle.class.canonicalName} verticle..."
		
		vertx.deployVerticle("groovy:" + EnronAppVerticle.class.canonicalName, 
			[
			...
			if(!res.succeeded()) {
				...
				System.err.println("${EnronAppVerticle.class.canonicalName} init failed: " + res.cause())
				System.exit(-1)
			}
			
			println "${EnronAppVerticle.class.canonicalName} started: ${res.result()}"
...
//		outboundPermitted : List<PermittedOptions>
		...
		def inboundPermitted1 = [
			address: ai.vital.auth.vertx3.VitalJSEndpointsManager.ENDPOINT_PREFIX + EnronAppVerticle.appID
		]
